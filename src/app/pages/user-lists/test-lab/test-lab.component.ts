...
reset(){
  ...
  ngOnInit() {
    ...
    this.Lab_info_form = this._formBuilder.group({
  ...
  addItem(): void {
    if(this.Lab_info_form.status == "INVALID"){
      Swal.fire({
        type: 'error',
      ...
      this.data.type = 'TESTLAB';
      this.data.nob = "TESTLAB";
      this.data.location ={
        "type":"point",
      ...
      this.ApiCall.postData('company', this.data).subscribe(data => {
        ...
        if (apidata.response.statusCode == 403) {
          ...
          Swal.fire({
            type: 'success',
            title: "TEST LAB added successfully.",
            timer: 10000
          });
        }
  ...
  nextStep(type){
    if(type == "address" && this.Lab_info_form.status == "INVALID"){
      Swal.fire({
        type: 'error',
        title: "Please enter required basic information.",
        timer: 10000
