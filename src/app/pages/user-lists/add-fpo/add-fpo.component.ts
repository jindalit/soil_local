...
export class AddFpoComponent implements OnInit {
  data: any = {};
  isLinear = false;
  firstFormGroup: FormGroup;
  secondFormGroup: FormGroup;
  FPO_info_form: FormGroup;
  ...
  ngOnInit() {
    ...
    this.FPO_info_form = this._formBuilder.group({
  ...
  getFile(params, type = null) {
    if (params.event.type == 4) {
      if (type == "testCertificate") {
        this.data.testCertificate.push({
  ...
  addItem(): void {
    if (this.FPO_info_form.status == "INVALID") {
      Swal.fire({
        type: 'error',
  ...
  nextStep(type) {
    if (type == "address" && this.FPO_info_form.status == "INVALID") {
      Swal.fire({
        type: 'error',
        title: "Please enter required basic information.",
        timer: 10000
